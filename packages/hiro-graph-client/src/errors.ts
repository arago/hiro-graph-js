...
const errors: Record<
  number,
  (message?: string, error?: ClientError) => ClientError
> = {};

const is = (code: number) => (err: ClientError) =>
  err.code && err.code === code;
const factory = (code: number, defaultMsg: string) => {
...
export const create = (code: number, msg: string, error = new Error()) => {
  if (code in errors) {
...
export const isUnknown = (err: ClientError) => !err.code || err.code / 100 >= 5;

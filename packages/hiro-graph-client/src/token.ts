...
// succeed.
export const cannotGetToken = (reason: string) => {
  return Object.assign(new Error(reason), { cannotGetToken: true });
};

export interface TokenOptions<Meta> {
...
export default class Token<Meta = any> {
  ...
  private _getToken: () => string | Promise<string>;
  private _getMeta: () => Meta;
...
export const fixedToken = (tokenString: string) =>

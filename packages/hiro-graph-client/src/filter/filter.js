...
Filter.prototype = {
    match: function (data) {
        ...
        var attrv = get(data, this.attrib);
...
/* Transform event data to correct format for matching */
Filter.transformEvent = function (event) {
    return omit(
        {
            ...event,
            element: {
...
Filter.matchString = function (data, filter) {
    ...
    if (filter.indexOf('*') < 0) {
        return match.some(function (cv) {
            if (cv) {
                return (
                    cv.toLowerCase() === Filter.unescape(filter).toLowerCase()
                );
            }
